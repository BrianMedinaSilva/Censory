...
int toggle_pro(Combatant& user) {
    ...
    if (user.name == "Bronson") {
        ...
        cout << "Peter's toggle, Distribute, has not yet been implemented.";
    }
    return multiplier;
}
...
void dialogue(std::string dialogue) {
    cout << dialogue << "\n";
    cin.get();
}
    ...
    while (living_player_team > 0 && living_enemy_team > 0) {
            ...
            if (player_team.team[i].alive == true) {
                while (player_team.team[i].actions > 0) {
                    ...
                    if (player_team.team[i].skills[command-1] == "Attack") {
                        ...
                        if (player_team.team[i].toggle == false) {
                            player_team.team[i].toggle = true;
                            cout << player_team.team[i].name << " activates Distribute (not yet implemented).\n";
                        }
                        else {
                            player_team.team[i].toggle = false;
                            cout << player_team.team[i].name << " deactivates Distribute (not yet implemented).\n";
                        }
                        continue;
                    }
                }
        ...
        for (int i = 0; i < enemy_team.team.size(); i++) {
            if (enemy_team.team[i].HP > 0) {
                ...
                else if (enemy_team.team[i].name == "Brainy Broom" || enemy_team.team[i].name == "Ambivalant" || enemy_team.team[i].name == "Sad Doctor" || enemy_team.team[i].name == "Louie") {
...
int tazewell(Team& player_team, Inventory& inventory) {
    ...
    while (in_tazewell == true) {
        int command = 0;
        cout << "What would you like to do? 1 to reassign stats, 2 to equip skills, 3 to check gradebook, 4 to leave Tazewell: ";
        cin >> command;
        while (command != 1 && command != 2 && command != 3 && command != 4) {
            cout << "Invalid! What would you like to do? 1 to reassign stats, 2 to equip skills, 3 to check gradebook, 4 to leave Tazewell: ";
            cin >> command;
        }
        if (command == 1) {
            int target = 0;
